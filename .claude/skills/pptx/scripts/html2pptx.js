...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      ...
      // On Linux, use full chromium instead of headless shell (which may lack libxkbcommon)
      const homePath = process.env.HOME || '/root';
      const cachePath = `${homePath}/.cache/ms-playwright/chromium-1200/chrome-linux64/chrome`;
      const fs = require('fs');
